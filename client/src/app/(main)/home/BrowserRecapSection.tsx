    ...
    async (userId: string, force = false) => {
      ...
      try {
        ...
        if (data.cached) {
          ...
          console.log("[BrowserRecap Client] REDIS CACHE MISS (Freshly generated by OpenAI)");
        }
        setRecapData(data);
        const now = Date.now();
  ...
  return (
    <div className="grid grid-cols-1 gap-6 mt-8">
      ...
      <div className="flex flex-col lg:flex-row gap-6 w-full items-stretch">
        ...
        <div className="rounded-xl border border-border bg-linear-to-br from-card to-purple-500/5 p-6 flex flex-col gap-4 shadow-xs relative overflow-hidden flex-1">
            ...
            <div className="flex flex-col divide-y divide-border/30">
              {upcomingTasks.map((task) => {
                ...
                return (
                  <div
                    key={task._id}
                    className="flex items-center justify-between py-3.5 first:pt-0 last:pb-0 hover:bg-purple-500/5 px-2 rounded-lg transition-colors group"
                  >
                    <div className="flex items-center gap-4 min-w-0 flex-1">
                      ...
                      {/* Separator line */}
                      <div className="h-8 w-[1px] bg-border/60 shrink-0" />

                      {/* Details Column */}
                      <div className="flex flex-col gap-1 min-w-0 flex-1">
                        <span className="text-sm font-semibold text-foreground truncate group-hover:text-purple-600 transition-colors">
                          {task.title}
                        </span>
